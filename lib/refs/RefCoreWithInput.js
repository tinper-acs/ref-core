...
var RefCoreWithInput = function (_Component) {
  ...
  function RefCoreWithInput(props) {
    ...
    _this.setFieldid = function () {
      try {
        var fieldid = _this.props.fieldid;

        var selectDom = _reactDom2["default"].findDOMNode(_this.selectRef);
        if (selectDom && fieldid) {
          selectDom.setAttribute('fieldid', fieldid + '_ref_core_with_input');
          var iconDom = selectDom.querySelector('.rc-tree-select-selection-menu-icon');
          var clearDom = selectDom.querySelector('.rc-tree-select-selection-choice-clear-icon');
          if (iconDom) {
    ...
    _this.onDropdownVisibleChange = function (open, documentClickClose) {
      if (open && !_this.state.showModal && _this.init) {
        _this.init = false;
        _this.onFilter(_this.state.searchValue);
      }
      var onDropdownVisibleChangeSelector = _this.props.onDropdownVisibleChangeSelector;
  ...
  RefCoreWithInput.prototype.render = function render() {
    ...
    var _props = this.props,
        ...
        selectorOpen = _props.selectorOpen,
        menuIcon = _props.menuIcon,
    ...
    return _react2["default"].createElement(
      ...
      _react2["default"].createElement(_menuSelectorTinperNext2["default"], _extends({
        style: {
          width: '100%'
        },
